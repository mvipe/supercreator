...
-- cascade automatically drops any policies attached to these tables,
-- so no separate "drop policy" statements are needed.
drop table if exists public.mp_notifications cascade;
drop table if exists public.mp_settings cascade;
drop table if exists public.mp_sub_orders cascade;
drop table if exists public.mp_bookings cascade;
drop table if exists public.mp_purchases cascade;
drop table if exists public.mp_orders cascade;
drop table if exists public.mp_availability cascade;
drop table if exists public.mp_sessions cascade;
drop table if exists public.mp_products cascade;
drop table if exists public.mp_courses cascade;
drop table if exists public.mp_profiles cascade;
...
create table if not exists public.mp_sub_orders (
  ...
  razorpay_order_id   text unique,
  razorpay_payment_id text,
  amount              integer not null default 49900, -- in paise (₹499)
  status              text not null default 'created' check (status in ('created','paid','failed')),
  created_at          timestamptz not null default now(),
  updated_at          timestamptz not null default now()
);
...
create index if not exists mp_sub_orders_user_idx on public.mp_sub_orders(user_id);
create index if not exists mp_sub_orders_created_idx on public.mp_sub_orders(created_at);
create index if not exists mp_sub_orders_razorpay_idx on public.mp_sub_orders(razorpay_order_id);
