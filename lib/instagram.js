...
export async function graph(path, { kind = "instagram", token, method = "GET", params, body } = {}) {
  ...
  try {
    ...
    if (!res.ok || json.error) {
      const e = json.error || {};
      // The request URL, with the token stripped, so a failing call is
      // debuggable from the error alone (surfaced on the callback screen).
      const safeUrl = url.toString().replace(/([?&])access_token=[^&]*/i, "$1access_token=REDACTED");
...
/**
 * The node insights are read from — always the numeric IG user id.
 *
 * `me/insights` on graph.instagram.com answers "Unsupported get request" (the
 * insights edge won't resolve the `me` alias, unlike `me/media`), so we use the
 * real id for both the Instagram-Login and Facebook-Page token flavours.
 */
const insightsNode = (account) => account.ig_user_id;
