...
/**
 * Staff = any admin: a super admin, an env-listed admin, OR a sub-admin
 * (mp_profiles.is_admin). Sub-admins have full admin panel access; only the
 * super admin can create/remove other admins. Resilient to a missing is_admin
 * column so the panel keeps working before subadmin.sql is run.
 */
export async function isStaff(user) {
  if (!user) return false;
  const phone = userPhone(user);
  if (phone && (phonesFrom(process.env.SUPER_ADMIN_PHONES).includes(phone) || phonesFrom(process.env.ADMIN_PHONES).includes(phone))) return true;
  let { data, error } = await supabaseAdmin
    .from("mp_profiles").select("is_super_admin, is_admin").eq("user_id", user.id).maybeSingle();
  if (error) {
    const r = await supabaseAdmin.from("mp_profiles").select("is_super_admin").eq("user_id", user.id).maybeSingle();
    data = r.data;
  }
  return !!(data?.is_super_admin || data?.is_admin);
}

/**
 * The workspace a request operates in. For a sub-admin (mp_team_members) this
 * is their owner's id, so anything they create/read belongs to the creator.
 * For a normal creator it's their own id. Resilient to a missing table.
 */
export async function getActiveOwnerId(user) {
  if (!user?.id) return null;
  try {
    const { data } = await supabaseAdmin.from("mp_team_members")
      .select("owner_id, active").eq("member_id", user.id).maybeSingle();
    if (data && data.active !== false) return data.owner_id;
  } catch { /* table not migrated — behave as a normal owner */ }
  return user.id;
}
