...
export function isProProfile(prof) {
  ...
  // Promoted platform admins (subadmin.sql) get Pro too — the PATCH that
  // promotes them also sets plan='pro', but an older promotion or a manual
  // DB edit could leave only the flag behind.
  if (prof.is_super_admin === true || prof.is_admin === true) return true;
