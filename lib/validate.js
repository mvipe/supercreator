...
const DEFAULT_TITLES = ["untitled course", "untitled", "your event title here", "untitled locked content", "untitled payment page", "your payment page title here", "untitled book"];
...
export function validateProduct(type, product = {}) {
  ...
  if (type === "payment") {
    ...
    // A payment page is a full sales page now — it needs cover art like a course.
    if (!(d.coverImages || []).length && isBlank(d.coverVideo)) issues.push("Add a cover image or video link");
  ...
  if (type === "book") {
    if (isBlank(d.description)) issues.push("Add a description");
    if (!(d.coverImages || []).length && isBlank(d.coverVideo)) issues.push("Add a cover image or video link");
    if (isBlank(d.fileUrl)) issues.push("Upload the book file, or paste a download link");
    if (d.priceMode === "fixed" && !(Number(d.price) > 0)) issues.push("Set a price greater than ₹0");
    if (d.priceMode === "pwyw" && !(Number(d.minPrice) > 0)) issues.push("Set a minimum price");
  }
