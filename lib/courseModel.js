...
/**
 * Poster/thumbnail for a video link, so a video card isn't a blank gradient.
 * YouTube and Vimeo both expose a predictable still; anything else returns
 * null and the caller falls back to its own artwork.
 */
export function videoThumb(url) {
  if (!url) return null;
  const y = String(url).match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([\w-]{6,})/);
